...
func init() {
	limiter.RegistLimiterBuilder("v1", newLimiterV1)
}
...
func newLimiterV1() limiter.Limiter {
	return &limiterV1{
		mutex: &sync.RWMutex{},
...
func (l *limiterV1) doRegistQuota(r *APIRegistQuotaReq) error {
	...
	if _, existed := l.meta.Get(rcTypeIdHex); existed {
		...
		l.meta.Set(rcTypeIdHex, limiter.NewLimiterMeta("v1", r.RCTypeID, r.Quota))
	}
...
func (l *limiterV1) recycle() {
	...
	for pair := range l.meta.Range(quitC) {
		limiterMeta := pair.V.(limiter.LimiterMeta)
		limiterMeta.Recycle()
	}
}
